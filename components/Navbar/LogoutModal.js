...
const LogoutModal = ({show, setShow}) => {
	...
	<h2 className="flex items-center gap-2 text-xl font-semibold leading-tight tracking-wide">
		<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" className="w-6 h-6 fill-current shrink-0 dark:text-lime-400">
		...
		</svg>Are you sure you want to logout?
	</h2>
	...
	<div className="flex flex-col justify-end gap-3 mt-6 sm:flex-row">
		<button onClick={() => setShow(false)} className="px-6 py-2 rounded-sm">No</button>
		<button onClick={logout} className="px-6 py-2 rounded-sm shadow-sm dark:bg-lime-400 dark:text-gray-900">Yes</button>
	</div>
</div>
  )
      return content
